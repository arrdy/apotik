<?php 
	...
	?>
                                ...
                                <table class="table table-bordered table-hover">
									...
									$exe = "
										select o.*,s.satuan,k.kategori,bo.ex,coalesce(bo.jml,0)as jml,nama,p.tgl,bo.harga as harga_beli
from obat o
left join kategori k on k.id_kategori=o.id_kategori
left join satuan s on s.id_satuan=o.id_satuan
left join (select faktur,kd_obat,sum(jumlah)as jml,max(tgl_kadaluarsa)as ex,harga from det_pembelian group by kd_obat,faktur)bo on bo.kd_obat=o.kd_obat
inner join pembelian p on p.faktur=bo.faktur
left join suplier sup on sup.id_suplier=p.id_suplier $cari
";
...
<form name='lpengadaan' method="post" action='laporan/print_pembelian.php?cari=<?php echo $cari; ?>' target='_blank'>
